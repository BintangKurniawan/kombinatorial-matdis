...
export default function Home() {
  ...
  return (
    <div className={`${balooBhaiBold.className} flex flex-col items-center justify-center gap-4 my-10`}>
          ...
          <div key={idx} className="flex flex-col gap-4">
            ...
            {list.map((val, i) => (
              <div key={i} className="flex gap-2">
                <input type="text" className="border border-gray-400 px-4 py-2 rounded-xl" placeholder={`Masukkan ${label.toLowerCase()}`} value={val} onChange={(e) => handleInputChange(e, i, list, setList)} />
                {i > 0 && (
                  <button className="bg-red-500 text-white px-4 rounded-xl" onClick={() => handleRemove(i, list, setList)}>
                    Hapus
                  </button>
                )}
              </div>
            ))}
            <button className="bg-blue-600 text-white px-4 py-2 rounded-xl mt-2 cursor-pointer" onClick={() => handleAdd(list, setList)}>
              Tambah {label}
            </button>
          </div>
        ))}
      ...
      <button onClick={handleBuatKombinasi} className="mt-8 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-xl cursor-pointer">
        Buat Kombinasi
      </button>
